...
ipcMain.handle('fetch-news', async () => {
    console.log("IPC: fetch-news called");
    ...
    console.log(`IPC: fetch-news returning ${result.length} items`);
...
ipcMain.handle('fetch-patch-notes', async () => {
    console.log("IPC: fetch-patch-notes called");
    try {
        ...
        console.log(`IPC: fetch-patch-notes returning ${result.length} items`);
        return result.map(entry => {
...
ipcMain.handle('fetch-content', async (event, url) => {
    console.log(`IPC: fetch-content called for ${url}`);
    if (!url) return null;
    try {
...
ipcMain.handle('get-versions', async () => {
    ...
    const filteredResult = result.filter(v => v.type === 'release' || v.type === 'snapshot');
    console.log(`IPC: get-versions returning ${filteredResult.length} items`);
    return filteredResult;
});

ipcMain.handle('get-fabric-versions', async () => {
    console.log("IPC: get-fabric-versions called");
    const data = await fetchJson('https://meta.fabricmc.net/v2/versions/game');
    const result = data ? data.map(v => ({ id: v.version, type: v.stable ? 'release' : 'snapshot' })) : [];
    console.log(`IPC: get-fabric-versions returning ${result.length} items`);
    return result;
});

ipcMain.handle('get-forge-versions', async () => {
    console.log("IPC: get-forge-versions called");
    ...
    if (data && data.promos) {
        ...
        const result = Array.from(mcVersions).sort((a, b) => b.localeCompare(a, undefined, { numeric: true })).map(v => ({ id: v, type: 'release' }));
        console.log(`IPC: get-forge-versions returning ${result.length} items`);
        return result;
    }
    console.log("IPC: get-forge-versions fetching failed or empty");
    return [];
});
...
function createSplashWindow() {
    splashWindow = new BrowserWindow({
        ...
        icon: fs.existsSync(localIconPath) ? localIconPath : undefined
    });
...
if (flowFileArg) {
    pendingFlowFile = flowFileArg;
}

app.on('second-instance', (event, argv) => {
    const flowArg = argv.find(arg => arg.endsWith('.flow') && fs.existsSync(arg));
    if (flowArg && mainWindow) {
        mainWindow.webContents.send('import-flow-file', flowArg);
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    createMainWindow();
                    if (pendingFlowFile && mainWindow) {
                        mainWindow.webContents.once('did-finish-load', () => {
                            mainWindow.webContents.send('import-flow-file', pendingFlowFile);
...
async function ensureJava(userDataPath, gameVersion) {
    ...
    // Mac creates a different structure usually (zulu-17.jdk/Contents/Home/bin/java)
    // For Linux/Win it's usually inside bin/
    ...
    if (isWin) {
        ...
        // Simple fallback for Mac (ARM/Intel logic omitted for brevity, assuming x64 or rosetta)
        ...
        if (extractedFolder) {
            ...
            try {
                if (fs.existsSync(javaDir)) {
                    // removing exist dir if needed, but for now assuming clean state or manual cleanup
                }
                ...
                // Return path inside extracted folder if rename fails
        ...
        if (fabricId) {
            ...
            if (modsForVersion) {
                ...
                if (app.isPackaged) {
                    ...
                    modJarSrc = path.join(__dirname, '..', 'flowcross-mod', 'build', 'libs', 'flowcross-client-1.0.0.jar');
                }
        ...
        launcher.on('data', (e) => {
            ...
            if (overlayWindow && !overlayWindow.isDestroyed()) {
                ...
                if (serverMatch) {
                    ...
                    overlayWindow.webContents.send('update-server', serverMatch[1]);
                } else if (logLine.match(/Starting integrated (minecraft )?server/i)) {
                    currentServerIp = null;
                    overlayWindow.webContents.send('update-server', "Local World");
                    ...
                    currentServerIp = null;
                    overlayWindow.webContents.send('update-server', "Not in World");
                }
            }
        });
...
try {
    ...
    console.log('electron-updater not installed, skipping auto-update initialization');
}
...
if (autoUpdater) {
    ...
    ipcMain.handle('check-for-updates', () => {
        if (process.env.NODE_ENV === 'development') {
            return { message: "Developement: Updates skipped" };
        }
